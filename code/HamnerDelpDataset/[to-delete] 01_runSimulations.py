...
@author:
    ...
    There are four processes included with this script:
        ...
        > Iterative RRA: three repeat iterations of RRA. The approach replicates
          ...
          kinematics are kept from IK each iteration while using an updated model [CHECK IF THIS STICKS...]
...
#NOTE: There may be some issues re-running certain parts of these pipelines when
#the data already exists, as there are potential difficulties in renaming files
...
for subject in subList:
    ...
    if runRRA3:
        ...
        try:
            os.mkdir(f'..\\..\\data\\HamnerDelp2013\\{subject}\\rra3')
        except:
            print(f'RRA3 folder already created for {subject}...')
            
        #Create run trial specific directory as well
        #Note this is currently just run5
        try:
            os.mkdir(f'..\\..\\data\\HamnerDelp2013\\{subject}\\rra3\\{runLabel}')
        except:
            print(f'RRA3 folder already created for {subject} and {runLabel}...')
    ...
    if runAddBiomech:
        try:
            os.mkdir(f'..\\..\\data\\HamnerDelp2013\\{subject}\\addBiomechanics')
        except:
            print(f'AddBiomechanics folder already created for {subject}...')
            
        #Create run trial specific directory as well
        #Note this is currently just run5
        try:
            os.mkdir(f'..\\..\\data\\HamnerDelp2013\\{subject}\\addBiomechanics\\{runLabel}')
        except:
            print(f'Moco folder already created for {subject} and {runLabel}...')
    ...
    if runRRA3:
        ...
        #Change to rra directory for ease of use with tools
        os.chdir(f'..\\..\\data\\HamnerDelp2013\\{subject}\\rra3\\{runLabel}')
        ...
        for rraIter in range(1,4):
            ...
            try:
                os.mkdir(f'rra{rraIter}')
            except:
                print(f'RRA folder already created for iteration {rraIter}...')
            ...
            for cycle in cycleList:
                ...
                #Add in opensim logger for cycle
                osim.Logger.removeFileSink()
                osim.Logger.addFileSink(f'{cycle}\\{runLabel}_{cycle}_rra3Log_{rraIter}.log')
                ...
                if rraIter == 1:
                    ...
                    rraTool.setModelFilename(f'..\\rra{rraIter-1}\\{cycle}\\{subject}_{runLabel}_{cycle}_rraAdjusted_iter{rraIter-1}.osim')
                    
                    #Use the adjusted RRA kinematics and don't filter
                    rraTool.setDesiredKinematicsFileName(f'..\\rra{rraIter-1}\\{cycle}\\{subject}_{runLabel}_{cycle}_iter{rraIter-1}_Kinematics_q.sto')
                    rraTool.setLowpassCutoffFrequency(-1)
                    
                    #Set the timings using the previous iteration kinematic data
                    rraTool.setInitialTime(osim.Storage(f'..\\rra{rraIter-1}\\{cycle}\\{subject}_{runLabel}_{cycle}_iter{rraIter-1}_Kinematics_q.sto').getFirstTime())
                    rraTool.setFinalTime(osim.Storage(f'..\\rra{rraIter-1}\\{cycle}\\{subject}_{runLabel}_{cycle}_iter{rraIter-1}_Kinematics_q.sto').getLastTime())
                ...
                osim.Logger.removeFileSink()
                #Read in the log file
                fid = open(f'{cycle}\\{runLabel}_{cycle}_rra3Log_{rraIter}.log', 'r')
                fileText = fid.readlines()
                fid.close()
                #Loop through the bodies
                for body in bodyList:
                ...
                #Adjust mass in the newly created model
                #Load the model
                rraAdjustedModel = osim.Model(f'{cycle}\\{subject}_{runLabel}_{cycle}_rraAdjusted_iter{rraIter}.osim')
                #Loop through the bodies and set the mass from the dictionary
                for body in bodyList:
                    #Get the new mass
                ...
                rraAdjustedModel.finalizeConnections()
                #Re-save the model
                rraAdjustedModel.printToXML(f'{cycle}\\{subject}_{runLabel}_{cycle}_rraAdjusted_iter{rraIter}.osim')
                ...
                #Set model file
                idTool.setModelFileName(f'{cycle}\\{subject}_{runLabel}_{cycle}_rraAdjusted_iter{rraIter}.osim')
                
                #Set the external loads file
                idTool.setExternalLoadsFileName(f'..\\..\\..\\expData\\{runName}_grf.xml')
                
                #Set the kinematics file from RRA
                idTool.setCoordinatesFileName(f'{cycle}\\{subject}_{runLabel}_{cycle}_iter{rraIter}_Kinematics_q.sto')
                
                #Set the time range
                idTool.setStartTime(osim.Storage(f'{cycle}\\{subject}_{runLabel}_{cycle}_iter{rraIter}_Kinematics_q.sto').getFirstTime())
                idTool.setEndTime(osim.Storage(f'{cycle}\\{subject}_{runLabel}_{cycle}_iter{rraIter}_Kinematics_q.sto').getLastTime())
                ...
                #Rename the body forces file
                os.rename(f'{cycle}\\body_forces_at_joints.sto', f'{cycle}\\{subject}_{runLabel}_{cycle}_iter{rraIter}_bodyForces.sto')  
    ...
    if runAddBiomech:
        ...
        #Change to rra directory for ease of use with tools
        os.chdir(f'..\\..\\data\\HamnerDelp2013\\{subject}\\addBiomechanics\\{runLabel}')
        ...
        genModel = osim.Model('..\\..\\model\\genericModel.osim')
        ...
        shutil.copyfile(f'..\\..\\expData\\{runName}.trc', f'{runName}.trc')
        shutil.copyfile(f'..\\..\\expData\\{runName}_grf.mot', f'{runName}_grf.mot')
        
        # #Extract cycles from broader trial file
        # for cycle in cycleList:
            
        #     #Read in the full trial TRC and MOT files to edit
        #     markerData = osim.TimeSeriesTableVec3(f'..\\..\\expData\\{runName}.trc')
        #     grfData = osim.TimeSeriesTable(f'..\\..\\expData\\{runName}_grf.mot')
            
        #     #Extract marker data for current cycle
            
        #     #Get row index related to start time in marker data
        #     startInd = np.argmin(np.abs(np.array(markerData.getIndependentColumn()) - gaitTimings[runLabel][cycle]['initialTime']))
            
        #     #Work up to the start index removing rows as progressing
        #     #Row index 0 is removed every time as the table is constantly updating
        #     #removing the first row each time
        #     for rowInd in range(startInd):
        #         markerData.removeRowAtIndex(0)
                
        #     #Get new row index related to final time in marker data
        #     stopInd = np.argmin(np.abs(np.array(markerData.getIndependentColumn()) - gaitTimings[runLabel][cycle]['finalTime'])) + 1
                
        #     #Work through remaining rows above the final time index. As above,
        #     #we can use the same index on each iteration as the table constantly
        #     #updates.
        #     for rowInd in range(stopInd+1,markerData.getNumRows()):
        #         markerData.removeRowAtIndex(markerData.getNumRows()-1)
                
        #     #Write new TRC data to file
        #     osim.TRCFileAdapter().write(markerData, f'{runLabel}_{cycle}.trc')
            
        #     #Extract GRF data for current cycle
            
        #     #Get row index related to start time in marker data
        #     startInd = np.argmin(np.abs(np.array(grfData.getIndependentColumn()) - gaitTimings[runLabel][cycle]['initialTime']))
            
        #     #Work up to the start index removing rows as progressing
        #     #Row index 0 is removed every time as the table is constantly updating
        #     #removing the first row each time
        #     for rowInd in range(startInd):
        #         grfData.removeRowAtIndex(0)
                
        #     #Get new row index related to final time in GRF data
        #     stopInd = np.argmin(np.abs(np.array(grfData.getIndependentColumn()) - gaitTimings[runLabel][cycle]['finalTime'])) + 1
                
        #     #Work through remaining rows above the final time index. As above,
        #     #we can use the same index on each iteration as the table constantly
        #     #updates.
        #     for rowInd in range(stopInd+1,grfData.getNumRows()):
        #         grfData.removeRowAtIndex(grfData.getNumRows()-1)
                
        #     #Write new TRC data to file
        #     osim.STOFileAdapter().write(grfData, f'{runLabel}_{cycle}.mot')
